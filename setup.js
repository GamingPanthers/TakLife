...
async function setupBot() {
    ...
    try {
        ...
        if (fs.existsSync('.env')) {
            ...
            if (existingEnv.DB_HOST) colorLog(`  - Database Host: ${existingEnv.DB_HOST}`, 'gray');
            if (existingEnv.DB_NAME) colorLog(`  - Database Name: ${existingEnv.DB_NAME}`, 'gray');
            if (existingEnv.GUILD_ID) colorLog(`  - Discord Server ID: ${existingEnv.GUILD_ID}`, 'gray');
            if (existingEnv.PORT) colorLog(`  - Web Panel Port: ${existingEnv.PORT}`, 'gray');
...
// Start setup
setupBot().catch(console.error);3000): ') || '3000';
